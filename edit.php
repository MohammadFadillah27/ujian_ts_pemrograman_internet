<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'GET'){
        ...
        $sql    = "SELECT * FROM tabel_siswa WHERE npsn=$npsn";
        ...
        $sk_izin_pendirian = $row["sk_pendirian"];
        $tgl_izin_pendirian    = $row["tgl_pendirian"];
        $sk_izin_oprasional        = $row["sk_izin_operasional"];
        $tgl_izin_oprasional       = $row["tgl_izin_operasional"];
        $alamat       = $row["alamat"];
        $rt           = $row["rt"];
        $rw           = $row["rw"];
        $dusun        = $row["dusun"];
        ...
        $sk_izin_pendirian = $_POST["sk_pendirian"];
        $tgl_izin_pendirian    = $_POST["tgl_pendirian"];
        $sk_izin_oprasional        = $_POST["sk_izin_operasional"];
        $tgl_izin_oprasional       = $_POST["tgl_izin_operasional"];
        $alamat       = $_POST["alamat"];
        $rt           = $_POST["rt"];
        $rw           = $_POST["rw"];
        $dusun        = $_POST["dusun"];
        ...
        do {
            if (empty($npsn) || empty($status) ||empty($bentuk_pendidikan) || empty($nama_sekolah) ||empty($sk_izin_pendirian) || empty($sk_izin_oprasional) ||empty($tgl_izin_oprasional) || empty($alamat) || empty($rt) || empty($rw) || empty($dusun) ||empty($desa) || empty($kecamatan) || empty($kabupaten) || empty($provinsi)|| empty($kode_pos)){
                $errorMessage = "Periksa Data yang Masih Kosong!";
                break;
            }
        
            $sql = "UPDATE tabel_siswa ". "SET status = '$status', bentuk_pendidikan = '$bentuk_pendidikan', nama_sekolah ='$nama_sekolah', sk_pendirian = '$sk_izin_pendirian', tgl_pendirian = '$tgl_izin_pendirian', sk_izin_operasional = '$sk_izin_oprasional', tgl_izin_operasional = '$tgl_izin_oprasional', alamat = '$alamat', rt = '$rt', rw = '$rw', dusun = '$dusun', desa = '$desa', kecamatan = '$kecamatan', kabupaten = '$kabupaten', provinsi = '$provinsi', kode_pos = '$kode_pos'".
                   "WHERE npsn = $npsn";
    ...
    <div class="container m-3">
        ...
        <form method="post">
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-6">
                    <input type="text" class="form-control" name="sk_pendirian" value="<?php echo $sk_izin_pendirian; ?>">
                </div>
            </div>
            <div class="row mb-3">
                <label class="col-sm-3 col-form-label">Tanggal Pendirian</label>
                <div class="col-sm-6">
                    <input type="text" class="form-control" name="tgl_pendirian" value="<?php echo $tgl_izin_pendirian; ?>">
                </div>
            </div>
            <div class="row mb-3">
                <label class="col-sm-3 col-form-label">SK Izin Operasional</label>
                <div class="col-sm-6">
                    <input type="text" class="form-control" name="sk_izin_operasional" value="<?php echo $sk_izin_oprasional; ?>">
                </div>
            </div>
            <div class="row mb-3">
                <label class="col-sm-3 col-form-label">Tanggal Izin Operasional</label>
                <div class="col-sm-6">
                    <input type="text" class="form-control" name="tgl_izin_operasional" value="<?php echo $tgl_izin_oprasional; ?>">
                </div>
            </div>
            <div class="row mb-3">
